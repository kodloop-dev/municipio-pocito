...
import { Plus, ChevronLeft, Send, ClipboardList, CircleDashed, Lightbulb, Trash2, TreePine, Hammer, MoreHorizontal, MapPin, type LucideIcon } from 'lucide-react'
...
function FormularioReporte({ onVolver, onEnviado }: { onVolver: () => void; onEnviado: () => void }) {
  ...
  async function handleEnviar() {
    if (!categoria) { toast.error('Seleccioná una categoría'); return }
    if (descripcion.trim().length < 10) { toast.error('Describí el problema (mínimo 10 caracteres)'); return }
    setEnviando(true)
    try {
